...
await p.locator('input.field').first().fill('vinyl');
await p.waitForTimeout(700);
const card = p.locator('text=Vinyl Banners').first().locator('xpath=ancestor::li[1]');
await card.waitFor({ state: 'visible', timeout: 10000 });
await card.locator('button:has-text("Edit")').click();
await p.waitForTimeout(600);
await p.locator('button:has-text("Pricing")').first().click();
await p.waitForTimeout(700);
await p.screenshot({ path: `${OUT}/05-editor-pricing-sqft-mobile-full.png`, fullPage: true });
await b.close();
console.log('done');
