...
const scroller = p.locator('div.overflow-y-auto').first();
// scroll to where "Price grid" heading is
const heading = p.locator('text=Price grid').first();
await heading.scrollIntoViewIfNeeded();
await p.waitForTimeout(400);
await p.screenshot({ path: `${OUT}/04b-matrix-grid-top-mobile.png` });
await scroller.evaluate(el => el.scrollBy(0, 500));
await p.waitForTimeout(300);
await p.screenshot({ path: `${OUT}/04c-matrix-grid-mid-mobile.png` });
await scroller.evaluate(el => el.scrollBy(0, 500));
await p.waitForTimeout(300);
await p.screenshot({ path: `${OUT}/04d-matrix-grid-bottom-mobile.png` });
await b.close();
console.log('done');
